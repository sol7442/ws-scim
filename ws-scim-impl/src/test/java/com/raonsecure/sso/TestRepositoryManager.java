...
public class TestRepositoryManager {
	...
	public void load() {
		try {
			if(SCIMResouceManager.getInstance().isLoad() == false) {
	...
	public void delete() {
		...
		try {
			SCIMResouceManager.getInstance().getRepositoryManger().deleteUser(decoed_user.getId());
	...
	public void create100() {
		...
		try {
			...
			while(count < 100) {
				...
				User created_user = (User)SCIMResouceManager.getInstance().getRepositoryManger().createUser(user);
