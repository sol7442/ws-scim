	...
	public void doExecute(JobExecutionContext context) throws SCIMException {
		...
		for (SCIMBulkOperation operation : operation_list) {
			...
			try {
				...
				e.printStackTrace();
				result.setStatus(SCIMError.BadRequest.getStatus());//.badre.br.b.SCIMType.uniqueness.toString());
...
//						operation, result);
	...
	private SCIMBulkOperation findResonseOperation(List<SCIMBulkOperation> response_opersations,
		...
		if (response_opersations == null) {
			...
			errer_operation.setStatus(SCIMError.InternalServerError.getStatus());//_ ode.SCIMType.serverError.toString());
			errer_operation.setResponse(SCIMError.InternalServerError);
