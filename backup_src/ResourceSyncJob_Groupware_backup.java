...
public class ResourceSyncJob_Groupware_backup {
	...
	public void afterExecute(JobExecutionContext context) throws SCIMException {
		...
		for (SCIMBulkOperation operation : operation_list) {
			...
			try {
				...
				e.printStackTrace();
				result.setStatus(SCIMError.InternalServerError.getStatus());//SCIMErrorCode.SCIMType.uniqueness.toString());
				result.setResponse(SCIMError.InternalServerError);
			} finally {
				scim_bluk_response.addOperation(result);
			}
		}
	...
	private SCIMBulkOperation findResonseOperation(List<SCIMBulkOperation> response_opersations, SCIMBulkOperation request_operation) {
		if(response_opersations == null) {
			...
			errer_operation.setStatus(SCIMError.InternalServerError.getStatus());//Code.SCIMType.serverError.toString());
